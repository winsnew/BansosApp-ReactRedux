...
import { addAid, editAid } from "../redux/aidAction";
...
function AidForm({ addAid, editAid, aidToEdit }) {
  ...
  const [formData, setFormData] = useState(
    aidToEdit ? aidToEdit : initialFormState
  );
  ...
  const handleSubmit = () => {
    if (editAid) {
      ...
      formData.id = new Date().getTime(); // Generate unique ID
      addAid(formData);
...
export default connect(null, mapDispatchToProps)(AidForm);
